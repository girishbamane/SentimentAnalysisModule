...
object SentimentAnalysisNew {
  ...
  def main(args: Array[String]): Unit = {
    ...
    rawStream.foreachRDD { kafkaRdd =>
      ...
      val myrdddf = kafkaRdd.map( ele =>  TweetRecord(ele._1.toString, ele._2.substring(0, ele._2.indexOf("\t")).toString,
      ...
      if(myrdddf.count()>0){
        println("MyRddCount : "+myrdddf.count())
        myrdddf.write.mode(SaveMode.Append).saveAsTable("twitterdb.twitter")
        counter = counter.toInt+myrdddf.count().toInt
